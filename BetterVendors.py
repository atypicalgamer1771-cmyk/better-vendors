...
class BetterVendors(SDKMod): 
    ...
    def _get_level(self, caller: unrealsdk.UObject) -> int: 
        try:
            ...
            # If game stage is 0 or very low, try to get player level instead
            if level <= 0:
                try:
                    pc = unrealsdk.GetEngine().GamePlayers[0].Actor
                    if pc and hasattr(pc, 'GetLevel'):
                        level = int(pc.GetLevel())
                except Exception:
                    pass
            return level if level > 0 else 72  # Default to high level so patches apply
        except Exception: 
            return 72  # Default to high level
    ...
    def _apply_patches(self, caller: unrealsdk.UObject) -> None: 
        ...
        unrealsdk.Log(f"[BetterVendors] Checking vendor - Level: {level}, Threshold: {threshold}")
        
        # Apply patches if above threshold or if using fallback level
        if level >= threshold or level == 72:
            ...
            unrealsdk.Log(f"[BetterVendors] Below threshold ({level} < {threshold}), skipping")
